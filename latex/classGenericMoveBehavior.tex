...
\begin{DoxyCompactList}\small\item\em step\+On moves on top of a target. \end{DoxyCompactList}\item 
...
step\+On moves on top of a target. 
...
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
true if it was able to. The step\+On function checks if everything in the target has a \mbox{\hyperlink{classMovement}{Movement}} behavior, and if it does it checks that everything is steppable. If either is false, it will not step on the target. 
\end{DoxyReturn}
